...
OCR_FILES_DIR300 = [5, 7, 8, 10, 12, 27, 28, 29, 31, 36, 53, 55, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73,
                    74, 85, 94, 96] + \
                   [103, 107, 108, 111, 115, 126, 128, 129, 130, 133, 135, 139, 140, 148, 149, 151, 159, 160, 161, 162,
                    163, 164, 165, 166, 167, 169, 170, 173, 174, 177] + \
                   [201, 202, 203, 205, 217, 218, 222, 223, 225, 227, 228, 237, 238, 239, 264, 265, 266, 271, 273, 277,
                    278, 285, 286, 288, 291, 294, 295, 296, 298, 300]
...
def OCR_eval_docunet(scan_path, preds_path, input_path):
    ...
    filename = "./eval/ocr_eval/docunet_ocr_files.txt"
    ...
    for file in tqdm(files):
        ...
        for i in [1, 2]:
            ...
            input_filepath = os.path.join(input_path, f"{file}_{i}.png")
...
def OCR_eval_DIR300(gt_path, preds_path, input_path):
    """
    Run OCR on the all DIR300 benchmark dataset.
    """
    # Run OCR on each samples
    CERs, EDs = [], []
    dict_results = {}
    for file in tqdm(OCR_FILES_DIR300):
        filepath = os.path.join(preds_path, f"{file}.png")
        scan_filepath = os.path.join(gt_path, f"{file}.png")
        input_filepath = os.path.join(input_path, f"{file}.png")
