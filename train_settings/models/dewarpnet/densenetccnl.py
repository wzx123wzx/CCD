...
class waspDenseEncoder128(nn.Module):
    def __init__(
        ...
        args=[0.2, False],
        f_activation=nn.Tanh, #nn.Tanh,nn.LeakyReLU,
        f_args=[],
    ):
        ...
        self.main = nn.Sequential(
            ...
            nn.BatchNorm2d(nc),
            nn.ReLU(True),
            # nn.Conv2d(nc+2, ndf, 4, stride=2, padding=1),
            ...
            DenseTransitionBlockEncoder(
                ndf * 4, ndim, 2, activation=activation, args=args
            ),
            # f_activation(*f_args),
            ...
            # DenseBlockEncoder(ndf * 8, 16),
            # DenseTransitionBlockEncoder(
            #     ndf * 8, ndim, 2, activation=activation, args=args
            # ),
            f_activation(*f_args),
        )
...
class waspDenseDecoder128(nn.Module):
    def __init__(
        ...
        activation=nn.ReLU,
        args=[False],
        f_activation=nn.Hardtanh, #nn.Hardtanh, nn.ReLU, 
        f_args=[],
    ):
        ...
        self.main = nn.Sequential(
            ...
            # nn.BatchNorm2d(nz),
            # activation(*args),
            # nn.ConvTranspose2d(nz, ngf * 8, 4, 1, 0, bias=False), # (7,7)
            # nn.ConvTranspose2d(nz, ngf * 8, 3, 2, 0, bias=False), # (9,9)
...
class dnetccnl(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # encoded = encoded#.unsqueeze(-1).unsqueeze(-1) # [4, 128, 1, 1]
        decoded = self.decoder(encoded) # [4, 2, 128, 128]
