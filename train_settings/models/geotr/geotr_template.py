...
#     train_options = {
#         "max_epochs": 300,
#         "batch_size": 2,  # virtual batch size of 8
...
#         bm_true = rearrange(batch["train"]["bm"], "b c h w -> b c w h") * 288 # [2, 2, 288, 288]
#         bm_pred = self.model(batch["input"]["image"], batch["input"]["template"]) # "template" # [2, 2, 288, 288]
...
#         bm_pred = self.model(batch["input"]["image"], batch["input"]["template"]) # "template"
...
class GeoTrTemplate(nn.Module):
    ...
    def forward(self, image1, template, return_local_feat=False):
        fmap1 = self.fnet1(
            image1
        )  # torch.Size([b, 3, 288, 288]) -> torch.Size([b, 128, 36, 36])
        ...
        coords1 = coords1.detach() # [2, 2, 36, 36]
        mask, coords1 = self.update_block(fmap, coords1) # [2, 576, 36, 36]
        flow_up = self.upsample_flow(coords1 - coords0, mask)
        bm_up = coodslar + flow_up
        if return_local_feat==True:
            return bm_up, fmap
        else:
            return bm_up
